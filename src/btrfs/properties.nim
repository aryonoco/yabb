...
const
  ...
  RequiredProps* = [PropUuid, PropTimestamp, PropType, PropParent, PropCompression,
                    PropChainPosition, PropChainLength]
...
proc setSnapshotMetadata*(path: string, meta: SnapshotMetadata, dryRun: bool = false): YabbResult[void] =
  ...
  setProperty(path, PropUuid, meta.uuid, dryRun).isOkOr:
    return err(error)
  setProperty(path, PropTimestamp, meta.timestamp.format("yyyy-MM-dd'T'HH:mm:ss'Z'"), dryRun).isOkOr:
    return err(error)
  setProperty(path, PropType, $meta.snapshotType, dryRun).isOkOr:
    return err(error)
  setProperty(path, PropParent, if meta.parent.isSome: meta.parent.get else: NoParentSentinel, dryRun).isOkOr:
    return err(error)
...
proc getSnapshotMetadata*(path: string): YabbResult[SnapshotMetadata] =
  ...
  let typeStr = getProperty(path, PropType).valueOr:
    return err(error)
  let snapshotType = case typeStr
    of "full": stFull
    of "incremental": stIncremental
    else: return err(btrfsError("Invalid snapshot type: " & typeStr))

  let parentStr = getProperty(path, PropParent).valueOr: NoParentSentinel
  let parent = if parentStr.len > 0 and parentStr != NoParentSentinel:
    Opt.some(parentStr)
  else:
    Opt.none(string)
  ...
  let hostname = if hostnameRes.isOk and hostnameRes.value.len > 0:
    Opt.some(hostnameRes.value)
  else:
    Opt.none(string)

  let sourceRes = getProperty(path, PropSource)
  let source = if sourceRes.isOk and sourceRes.value.len > 0:
    Opt.some(sourceRes.value)
  else:
    Opt.none(string)

  let kernelRes = getProperty(path, PropKernel)
  let kernel = if kernelRes.isOk and kernelRes.value.len > 0:
    Opt.some(kernelRes.value)
  else:
    Opt.none(string)

  let fsUuidRes = getProperty(path, PropFsUuid)
  let fsUuid = if fsUuidRes.isOk and fsUuidRes.value.len > 0:
  ...
  let chainLenStr = getProperty(path, PropChainLength).valueOr: "0"
  let chainLen = try: parseInt(chainLenStr) except ValueError: 0

  let sizeStr = getProperty(path, PropSizeBytes).valueOr: "0"
  let sizeBytes = try: parseBiggestInt(sizeStr) except ValueError: 0i64
...
proc getChainPosition*(path: string): int =
  ...
  if res.isOk:
    try: return parseInt(res.value)
    except ValueError: discard
  0

proc updateChainLength*(snapshotDir: string, length: Natural, dryRun: bool = false): YabbResult[void] =
  ...
  if dryRun:
    debug "DRY_RUN: Would update chain length", snapshotDir = snapshotDir, length = length
    return ok()

  let entries = try: toSeq(walkDir(snapshotDir)) except OSError: @[]
  entries.filterIt(it.kind == pcDir and extractFilename(it.path).startsWith(SnapshotPrefix)).applyIt:
