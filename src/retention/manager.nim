...
proc keepCurrentPartialHour*(
  ...
  let currentHourStart = dateTime(
    referenceTime.year, referenceTime.month, referenceTime.monthday,
    referenceTime.hour, 0, 0, zone = utc()
  )
  let currentHourEpoch = currentHourStart.toTime.toUnix

  # Filter snapshots in current hour
  let snapshotsToKeep = snapshots.filterIt(it.timestamp.toTime.toUnix >= currentHourEpoch)
...
proc selectSnapshotsToKeep*(
  ...
  proc applyRetentionPeriod(
    ...
    let pathsToAdd = periodsToCheck.mapIt(block:
      let (periodStart, periodEnd) = getPeriodBoundaries(period, it, referenceTime)
      let inPeriod = sorted.filterIt(isInPeriod(it.timestamp, periodStart, periodEnd))
      ...
      let (checkResult, latestPath) =
        if inPeriod.len == 0:
          ...
          if latest.path in currentSet: (rprAlreadyKept, latest.path)
          else: (rprNewSnapshot, latest.path)
  ...
  let withHourly = applyRetentionPeriod(withPartialHour, rpHourly, retention.hourly, "hourly", sorted, referenceTime)
  let withDaily = applyRetentionPeriod(withHourly, rpDaily, retention.daily, "daily", sorted, referenceTime)
  let withWeekly = applyRetentionPeriod(withDaily, rpWeekly, retention.weekly, "weekly", sorted, referenceTime)
  let withMonthly = applyRetentionPeriod(withWeekly, rpMonthly, retention.monthly, "monthly", sorted, referenceTime)
  applyRetentionPeriod(withMonthly, rpYearly, retention.yearly, "yearly", sorted, referenceTime)
...
proc applyRetention*(
  ...
  let (deleted, deletedList) = toDeleteSnapshots.foldl(
    block:
      ...
      let outcome =
        if config.dryRun: doDryRun
        elif deleteSubvolume(b.path, config.dryRun).isOk: doSuccess
        else: doFailure
...
proc getRetentionSummary*(
  ...
  ok((
    total: snapshots.len,
    toKeep: toKeep.len,
    toDelete: snapshots.len - toKeep.len
  ))
